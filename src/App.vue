...
function answerClasses(answer) {
  if (!isAnswerRevealed.value) {
    ...
    return 'bg-slate-800 border-slate-600 hover:bg-slate-700'
  }
...
<template>
  <div class="min-h-screen bg-#29165B-950 text-white flex items-center justify-center p-4">
    
    <!-- SCHERMATA INIZIALE -->
    <div v-if="gameStatus === 'idle'" class="text-center">
      <h1 class="text-4xl font-bold mb-4">Chi Vuol Essere Milionario</h1>
      <p class="text-slate-400 mb-6">{{ errorMessage }}</p>
      <button
        @click="startGame"
        class="bg-blue-600 hover:bg-blue-700 px-8 py-3 rounded-lg text-lg font-semibold"
      >
        Inizia
      </button>
    </div>
    ...
    <!-- CARICAMENTO -->
    <div v-else-if="gameStatus === 'loading'" class="text-center">
    ...
    <div v-else-if="gameStatus === 'playing' && currentQuestion" class="w-full max-w-2xl">
      ...
      <button
        ...
        @click="useFiftyFifty"
        class="mb-4 bg-purple-700 hover:bg-purple-800 px-4 py-2 rounded text-sm"
      >
        Aiuto 50:50
      </button>
  ...
  @click="useHint"
  class="mb-4 ml-2 bg-purple-700 hover:bg-purple-800 px-4 py-2 rounded text-sm"
>
  Chiedi al genio
</button>
      ...
      <div class="grid grid-cols-1 gap-3">
        <button
          ...
          class="border-2 rounded-lg px-4 py-3 text-left transition-colors"
        >
          {{ answer }}
        </button>
      </div>
